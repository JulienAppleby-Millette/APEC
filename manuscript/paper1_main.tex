...
studies \cite{Ghosez1998_BEC,Zhong1994_BTO,Veithen2005} where  $Z^*_{zz} = 7.29$~$e$. In the tetragonal phase, the 
...
A limited leave-one-out calibration test of the phase/component-consistent subsets of the BaTiO$_3$ literature library is presented in Supplemental Material Section~S3.7. Across 15 residuals in four computational groups, the pooled residuals give mean $z=-0.02$, RMS $z=1.05$, reduced $\chi^2=1.10$, 73\% coverage for $|z|<1$, and 93\% coverage for $|z|<1.96$. The expected sampling spread of the RMS statistic is approximately $1/\sqrt{2n}=0.18$, so these values rule out gross miscalibration but do not distinguish fine calibration differences. This is an Amber-level smoke test for gross over- or under-confidence in the BaTiO$_3$ input library, not a broad calibration benchmark across materials or a direct empirical validation of the derived Red-tier $r_{33}$ certificate.
