...
For ideal bulk unit cells, density-functional perturbation theory and related
finite-field approaches can compute electronic, ionic, and piezoelectric
contributions to the Pockels response directly
\cite{Veithen2005,Veithen2004_PRL}. The scientific difficulty addressed here
is different: the regimes that make BaTiO$_3$ technologically compelling are
not single perfect bulk crystals, but strained, domain-textured,
defect-bearing, temperature-dependent, and heterostructured environments. In
those settings, a direct full-tensor calculation at every relevant
configuration would require repeated response calculations on large,
low-symmetry cells or ensembles, making it poorly suited as the primitive
object for uncertainty-aware aggregation and propagation. A calibrated
soft-mode description is therefore useful not because the full tensor is
unknowable, but because it provides a compact, physically interpretable
descriptor whose assumptions, calibration anchor, and uncertainty
amplification can be certified explicitly.
...
Here, we introduce a methodology that treats uncertainty as structured metadata attached to computational materials data. The specific target is the record-level labelling problem in existing DFT data: values enter literature compilations and databases without per-record uncertainty, machine-actionable provenance, or exchangeability metadata, so quantities with different reliability, tensor conventions, physical scopes, and systematic biases can be compared, fused, or used as training labels as if they were equivalent. Rather than being tied to a particular exchange-correlation functional, workflow or dataset, the resulting A Posteriori Error Certificates (APECs) can be generated retrospectively for completed calculations and incorporated into existing databases. Each computed quantity of interest is provided with an uncertainty certificate. The certificate decomposes uncertainty into convergence, methodological, intrinsic DFT and model-form contributions, records computational provenance, and assigns a quality tier. The approach also provides a means of propagating these uncertainties through derived quantities and combining results across heterogeneous sources in a consistent manner. By making label uncertainty and exchangeability explicit, APECs define the data-side substrate on which later model-selection or machine-learning calibration questions can be posed rigorously; those model-side questions require separate validation and are not claimed here.
...
based on the ranges summarized in Supplemental Material Table~S4, shown as visual interpolants rather than as explicit DFPT convergence calculations at every point. Only the starred points correspond to evaluations at the production-calculation parameters; the connecting curves are guides constrained to reproduce the class-level ranges of Supplemental Material Table~S4 and are not point-by-point DFPT results. The soft-mode frequency
...
BaTiO$_3$. Curves are visual interpolants constrained to reproduce the class-level DFPT uncertainty ranges summarized in Supplemental Material Table~S4; they are not explicit DFPT convergence calculations at every point and are not fitted from the preliminary transferable convergence parameters in Supplemental Material Table~S3. Only the starred points correspond to evaluations at the production-calculation parameters (PBE/PAW, 60~Ry, $6^3$ $k$-points). The green band indicates the $<2\%$ convergence region. }
...
sensitivity is discussed in the Supplemental Material: substituting the raw
...
subsets of the BaTiO$_3$ literature library is presented in Supplemental
Material Section~S3.7. Across 19 residuals in five computational subsets, the
...
(Supplemental Material Table~S5): the soft-mode and cubic $Z^*_{\mathrm{Ti}}$
...
The results presented in the BaTiO$_3$ case study illustrate several distinct roles of APECs. The clearest concrete finding is that provenance-aware certification exposed a labelling failure that a conventional literature average would have hidden. When recent tetragonal-phase entries were added to an unpartitioned library, the fused uncertainty in $\varepsilon^\infty$ widened rather than narrowed, indicating that physically distinct quantities were being averaged under one label. Phase and tensor-component provenance localized the source of the inconsistency, and correcting the exchangeability class changed the Born-charge contribution to the derived electro-optic response by roughly 13\%. This is not merely bookkeeping: widening-on-addition is a diagnostic that the data label is too coarse for the quantity being inferred.
More broadly, APECs quantify uncertainty associated with individual DFT-computed and derived quantities by decomposing it into convergence, methodological, intrinsic DFT, and model-form contributions. They enable uncertainty-aware fusion of heterogeneous data sources, propagate uncertainty through nonlinear physical relationships, and identify where additional information can most effectively improve predictions. In the present example, numerical convergence contributes relatively little to the total uncertainty of the electro-optic response, whereas methodological and model-form contributions dominate. Consequently, additional convergence studies are expected to yield only marginal improvements in uncertainty. In contrast, new cross-functional calculations, direct electro-optic tensor calculations, and improved calibration data would reduce uncertainty more substantially. These uses make uncertainty quantification an effective tool for guiding further computational and experimental effort.
...
experimental reference data. The band-gap case study in the Supplemental
Material provides the clearest example of this broader functional-family
...
single-calculation certificates and aggregated literature data can provide useful diagnostic information. In the present work, the widening of the $\varepsilon^\infty$ posterior under unpartitioned aggregation was resolved by inspecting the phase and tensor-component metadata and separating non-exchangeable records. The certificate framework does not diagnose such
errors automatically in its present implementation, but it creates the conditions under which they
become visible and supplies the provenance fields needed for automated checks.
...
The structured nature of the certificate metadata also creates opportunities beyond uncertainty quantification for individual calculations. Recent work in materials informatics has highlighted the importance of uncertainty-aware machine learning, particularly when training models on heterogeneous datasets assembled from multiple computational and experimental sources \cite{Kendall2017,Nix1994,Guo2017_calibration,Kuleshov2018,Tran2018_active}. Most predictive-uncertainty methods focus on model uncertainty, while the training labels themselves are often treated as exact. APECs address the complementary label channel: each record carries a per-label uncertainty, provenance, exchangeability class, and, where data permit, a method-dependent bias estimate. These quantities can support inverse-variance weighting, tier-aware filtering, bias-corrected training sets, and calibration audits without assuming that all DFT labels are equally reliable. We do not claim here that a particular learning architecture improves when trained on certificates; such a claim requires a separate, chemically disjoint benchmark study. The contribution of the present work is the certified label distribution and provenance structure needed to make that benchmark well posed \cite{Schutt2018_SchNet,Chen2022_M3GNet,Batatia2022_MACE}.
...
and is nonzero. Implementation details are provided in Supplemental Material Sections~S2.2--S2.3 and the support repository documentation. 
...
library (Supplemental Material Table~S6), the three quantities entering the soft-mode expression for $r_{33}$ shift systematically with the choice of functional. PBE gives slightly larger Born effective charges ($+5\%$) and dielectric constants ($+8\%$) than LDA, while the soft-mode frequency changes comparatively little.  Because contributions from $Z^*$ and $\varepsilon^\infty$ enter $r_{33} \propto
...
$\omega_{\mathrm{soft}}$, $Z^*$, and $\varepsilon^\infty$  (see Supplemental Material Section~S1.1).
...
Supplemental Material Table~S3. These preliminary values define the scope of
